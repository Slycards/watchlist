...
app.get("/api/player/:id", async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      const code = person?.status?.code;      // "A"=active, "D"/"DL"/"D7"/"D10"/"D15"/"D60"=IL variants
      const desc = person?.status?.description || "";
      const onIL = code && code !== "A" && /injured|IL|disabled/i.test(desc + code);
      if (onIL) {
        // derive a short label like "IL-15" / "IL-60" from the description
        const m = desc.match(/(\d+)\s*-?\s*day/i);
        const label = m ? `IL-${m[1]}` : "IL";
        // find the "since" date from the team's full roster entry
        let since = "";
    ...
    } catch { /* no IL data → il stays null */ }
