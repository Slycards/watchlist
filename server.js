...
app.get("/api/player/:id", async (req, res) => {
  ...
  try {
    const [seasonJ, logJ, statusJ] = await Promise.all([
      ...
      // player status (Active vs IL); hydrate currentTeam so we can find the
      // roster entry for the IL type + date if needed.
      fetchJson(`${MLB}/v1/people/${id}?hydrate=currentTeam`).catch(() => null),
    ]);

    // ---- injured-list status (reliable current status; "since" from roster) ----
    let il = null;
    try {
      const person = statusJ?.people?.[0];
      const code = (person?.status?.code || "").toUpperCase();
      const desc = person?.status?.description || "";
      console.log(`[il] player ${id} statusCode="${code}" desc="${desc}"`);
      // Active codes are "A" (active) and "" (unknown). Anything else is a
      // non-active status; IL codes start with "D" (D7/D10/D15/D60) or mention
      // injured/IL/disabled in the description.
      const onIL = (code.startsWith("D")) || /injured|(\bIL\b)|disabled/i.test(desc);
      if (onIL) {
        const m = desc.match(/(\d+)\s*-?\s*day/i);
        // fall back to the numeric part of the code (D15 → 15) if desc has no "N-day"
        const codeNum = code.match(/D(\d+)/);
        const days = m ? m[1] : (codeNum ? codeNum[1] : "");
        const label = days ? `IL-${days}` : "IL";
        let since = "";
        const teamId = person?.currentTeam?.id;
        if (teamId) {
          try {
            const rj = await fetchJson(`${MLB}/v1/teams/${teamId}/roster?rosterType=fullRoster&hydrate=person`);
            const entry = (rj.roster || []).find((r) => r.person?.id === id);
            since = entry?.status?.date || entry?.startDate || "";
          } catch { /* no since date */ }
        }
        il = { label, desc, since };
        console.log(`[il] player ${id} ON IL → ${label} since="${since}"`);
      }
    } catch (e) { console.log(`[il] player ${id} status check failed:`, e.message); }
    ...
    const payload = { id, season, splits, lastGame, il };
