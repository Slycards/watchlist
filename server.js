...
app.get("/api/player/:id", async (req, res) => {
  ...
  try {
    ...
    // "super" indicator — ALL THREE windows independently land on the same
    // true extreme (Hot or Cold, not the softer Heating Up/Cooling bands).
    // The per-window bars themselves were lowered (see HEAT_THRESHOLDS) so
    // this stays achievable in practice, while keeping the meaningful
    // "consistent across every window" structure rather than a majority rule.
    let superHeat = null;
    const keys = [splits.d7.heat?.key, splits.d15.heat?.key, splits.d30.heat?.key];
    if (keys.every(k => k === "hot")) superHeat = "hot";
    else if (keys.every(k => k === "cold")) superHeat = "cold";
