...
app.get("/api/player/:id", async (req, res) => {
  ...
  try {
    ...
    // "super" indicator — follows the actual shape of a breakout, not just
    // matching magnitudes. Two distinct trend shapes both qualify:
    //   Pattern A: an established/building streak — 30-day at least Heating
    //     Up, fully Hot at 15 and 7 days (this also fully includes the
    //     simplest case of "Hot at all three windows").
    //   Pattern B: a fresh, very recent-onset streak — 30-day still Steady
    //     (the full month hasn't moved yet), but a clear escalating staircase
    //     in the more recent windows: Heating Up at 15 days, fully Hot at 7.
    // Both describe genuine acceleration toward the present, just starting
    // from a different point — not just three windows happening to match.
    let superHeat = null;
    const d30key = splits.d30.heat?.key;
    const d15key = splits.d15.heat?.key;
    const d7key = splits.d7.heat?.key;
    const hotA = (d30key === "heating" || d30key === "hot") && d15key === "hot" && d7key === "hot";
    const hotB = d30key === "steady" && d15key === "heating" && d7key === "hot";
    const coldA = (d30key === "cooling" || d30key === "cold") && d15key === "cold" && d7key === "cold";
    const coldB = d30key === "steady" && d15key === "cooling" && d7key === "cold";
    if (hotA || hotB) superHeat = "hot";
    else if (coldA || coldB) superHeat = "cold";

    // Heat Meter Score — a single 0-10 number for the Season tab, continuous
    // rather than banded. Each window scores 5 (neutral) plus or minus up to
    // 5, scaled by how far its delta sits relative to THAT window's own Hot/
    // Cold threshold — so a player who just barely crossed into "Hot" scores
    // differently than one having a monster week, even though both would
    // show the same "Hot" label on the gauge. Averages only the windows that
    // actually have a reading; null if none do.
    let heatScore = null;
    if (season) {
      const windowScores = [];
      for (const w of [splits.d7, splits.d15, splits.d30]) {
        if (!w.heat) continue;
        const raw = 5 + 5 * (w.heat.delta / w.heat.edge);
        windowScores.push(Math.max(0, Math.min(10, raw)));
      }
      if (windowScores.length) {
        const avg = windowScores.reduce((a, b) => a + b, 0) / windowScores.length;
        heatScore = Math.round(avg * 10) / 10;
      }
    }
    ...
    const payload = { id, season, splits, lastGame, il, level: level.label, superHeat, heatScore };
