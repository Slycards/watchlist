...
app.get("/api/player/:id", async (req, res) => {
  ...
  try {
    ...
    // ---- injured-list status ----
    // /people/{id} returns NO status (logs confirmed empty), so read it from the
    // team's 40-man roster, which reliably carries each player's status + date.
    // Roster is cached per team (1h) so multiple players on one team = one fetch.
    let il = null;
    try {
      ...
      if (teamId) {
        ...
        console.log(`[il] player ${id} rosterStatus code="${code}" desc="${desc}"`);
        ...
        if (onIL) {
          ...
          il = { label: days ? `IL-${days}` : "IL", desc, since: entry?.status?.date || "" };
          console.log(`[il] player ${id} ON IL → ${il.label}`);
        }
      }
