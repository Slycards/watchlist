...
app.get("/api/player/:id", async (req, res) => {
  ...
  try {
    ...
    // ---- injured-list status (reliable part only: label, no since-date) ----
    // Read from the team's full roster, which reliably carries status; the
    // /people/{id} endpoint does not. Roster is cached per team (1h).
    let il = null;
    try {
