...
// Gzip every response — JSON compresses very well (often 70-85% smaller),
// and this app is largely JSON API responses, so this directly cuts how
// much data has to travel over the network on every request. Placed first
// so it wraps everything below it, including the static file serving.
app.use(compression());
          ...
          await Promise.all(missing.map(async (id) => {
            try {
              ...
              if (teamId) {
                ...
                console.log(`[live] id=${id} team resolution returned no currentTeam.id — raw people[0]: ${JSON.stringify(pj.people?.[0])}`);
              }
            } catch (e) {
              console.log(`[live] id=${id} team resolution fetch threw:`, e.message);
            }
          }));
        }
